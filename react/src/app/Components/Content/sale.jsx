      ...
      <div className="overflow-hidden px-2" ref={emblaRef}>
        ...
        <div className="flex">
          {sales.map((s, i) => (
            ...
            >
              <div className={`${s.color} rounded-xl px-6 py-9 justify-between h-full flex`}>
                <div className="flex flex-col gap-3 w-[50%]">
                  <h3 className="text-xl font-bold">{s.title}</h3>
                  <p className="text-green-700 dark:text-green-950 font-semibold text-xl">{s.discount}</p>
                  <p className="text-gray-600 dark:text-gray-950">{s.desc}</p>
                  <button className="mt-3 bg-green-600 dark:bg-green-800 text-white px-4 py-2 rounded-md w-fit">Shop Now</button>
                </div>
              <Image src={s.image} alt={s.title} width={150} height={120} className="rounded-[50%] object-cover" />
              </div>
            </div>
          ))}
        </div>
