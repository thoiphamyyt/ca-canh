...
export default function FormCreateProduct({
  ...
  const loadDetailProduct = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        // Nếu product có images từ DB
        if (product.images && Array.isArray(product.images)) {
          setImages(
            product.images.map((url, idx) => ({
              id: idx,
              name: url.split("/").pop(),
              url: url, // link ảnh từ server
              file: null, // không có file thật vì ảnh từ server
            }))
          );
        }
      }
  ...
  return (
    <ComponentCard title={isUpdate ? "Cập nhật sản phẩm" : "Thêm mới sản phẩm"}>
